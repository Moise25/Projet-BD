...
if(strcmp(argv[2],"json")==0 ) {
	if(argc==8) { //l'option -t est presente
		if ( strcmp(argv[4],"-f")==0 ) { //fichier en input
			...
			snprintf(commande1, 256, "valid-extract_json/./valid_json %s > generation_json", argv[5]); //envoie les données dans une chaine de caractères
			int status = system(commande1);
			//affichage trace
			snprintf(commande2, 256, "valid-extract_json/./valid_json %s", argv[5]);
			int status2 = system(commande2);
		}
		if ( strcmp(argv[4],"-h")==0 ) { //fichier en input
			char commande1[256] , commande2[256];
			snprintf(commande1, 256, "valid-extract_json/./valid_json %s > generation_json", argv[5]);
			int status = system(commande1);
			//affichage trace
			snprintf(commande2, 256, "valid-extract_json/./valid_json %s", argv[5]);
			int status2 = system(commande2);
	...
	if(argc==7) { //l'option -t est absente
		if ( strcmp(argv[3],"-f")==0 ) { //fichier en input
			char commande1[256] ;
			snprintf(commande1, 256, "valid-extract_json/./valid_json %s > generation_json", argv[4]);		
			int status = system(commande1);
		}
